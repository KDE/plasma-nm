...
QQC2.ItemDelegate {
    ...
    QQC2.ContextMenu.menu: connectionItemMenu
    ...
    function changeState() {
        if (!predictableWirelessPassword && (model.ConnectionState == PlasmaNM.Enums.Deactivated || model.ConnectionState === PlasmaNM.Enums.Deactivating)) {
            if (!predictableWirelessPassword && !model.Uuid) {
                kcm.handler.addAndActivateConnection(model.DevicePath, model.SpecificPath);
            } else {
                kcm.handler.activateConnection(model.ConnectionPath, model.DevicePath, model.SpecificPath);
            }
        } else if (!predictableWirelessPassword) {
            kcm.handler.deactivateConnection(model.ConnectionPath, model.DevicePath);
        } else if (predictableWirelessPassword) {
