...
NMVariantMapMap OpenVpnUiPlugin::importConnectionSettings(const QString &fileName)
{
    ...
    NMConnection *connection = nm_vpn_editor_plugin_import(plugin, fileName.toStdString().c_str(), &error);

    NMVariantMapMap result;

    const QString connectionName = QFileInfo(fileName).completeBaseName();

    NMSettingVpn *vpnSetting = nm_connection_get_setting_vpn(connection);

    NMStringMap vpnData;
    nm_setting_vpn_foreach_data_item(
        vpnSetting,
        [](const char *key, const char *val, gpointer user_data) {
            static_cast<NMStringMap *>(user_data)->insert(key, val);
        },
        &vpnData);
    ...
    int routeCount = nm_setting_ip_config_get_num_routes(ipconfig);

    QList<NetworkManager::IpRoute> list;
    for (int i = 0; i < routeCount; i++) {
        NMIPRoute *route = nm_setting_ip_config_get_route(ipconfig, i);

        const char *dest = nm_ip_route_get_dest(route);
        int prefix = nm_ip_route_get_prefix(route);

        NetworkManager::IpRoute ipRoute;
        ipRoute.setIp(QHostAddress(QString::fromUtf8(dest)));
        ipRoute.setPrefixLength(prefix);
        list << ipRoute;
    }
